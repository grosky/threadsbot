...
> **Сопутствующие файлы:** `STRATEGY.md` — бизнес-стратегия, цели, открытые вопросы; `TODO.md` — продуктовые задачи; `APP_REVIEW.md` — материалы для Meta.
...
│   ├── profile_packaging.py# Упаковка профиля с нуля (имя/bio/ссылка/закреп)
...
- **Упаковка профиля** хранится в `users.profile_pack_json` (одна запись на юзера, перезаписывается при перегенерации блока). Не списывает дневной лимит. Доступ — по подписке или free trial.
